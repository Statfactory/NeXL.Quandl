...
let quandlDir = Path.Combine(sourceDir, "NeXL.Quandl")

let xlsmPath = Path.Combine(quandlDir, "Quandl.xlsm") // path to Excel workbook


// Directories
let buildDir  = Path.Combine(quandlDir, "bin/Release")

let path32BitXll = Path.Combine(quandlDir, sprintf "NeXL32bit_%d_%d_%d.xll" nexlVer.Major nexlVer.Minor nexlVer.Build)
let path64BitXll = Path.Combine(quandlDir, sprintf "NeXL64bit_%d_%d_%d.xll" nexlVer.Major nexlVer.Minor nexlVer.Build)
...
Target "Build" (fun _ ->
    ...
    WorkbookComPackager.embedCustomization(buildDir, quandlDir, "")
)


Target "Embed" (fun _ ->
        let custAssemblyPath = Path.Combine(quandlDir, @"bin\release\NeXL.Quandl.dll")
        let asmBytes = File.ReadAllBytes(custAssemblyPath)
        let ref1AsmBytes = File.ReadAllBytes(Path.Combine(quandlDir, @"bin\release\FSharp.Data.dll"))
        let ref2AsmBytes = File.ReadAllBytes(Path.Combine(quandlDir, @"bin\release\Newtonsoft.Json.dll"))
        ...
        WorkbookPackager.EmbedProdID(xlsmPath, "1B48CBFE-A7B5-41F7-A7BA-3CCD4ABB1FD2")
